...
- 工具、插件、命令均支持白名单、黑名单和数量上限。
- 白名单和黑名单直接使用 AstrBot 原生插件设置中的动态多选控件。
...
白名单和黑名单直接在插件设置界面中多选。选项来自当前可用工具、已启用插件和命令，并会在 AstrBot 启动以及插件加载、卸载后刷新。白名单为空表示允许全部；同一项同时出现在两侧时，黑名单优先。
