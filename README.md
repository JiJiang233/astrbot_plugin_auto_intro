...
- 保存插件白名单时，会自动把所选插件注册的全部命令并入命令白名单。
...
白名单和黑名单直接在插件设置界面中多选。选项来自当前可用工具、已启用插件和命令，并会在 AstrBot 启动以及插件加载、卸载后刷新。插件白名单非空时，保存配置会把所选插件注册的全部命令自动并入命令白名单，已有的手动命令选择会保留；取消选择插件后，只移除由该联动自动加入的命令。白名单为空表示允许全部；同一项同时出现在两侧时，黑名单优先。
